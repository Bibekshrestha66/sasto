...
describe("Review System", () => {
  const testUserId1 = 900001;
  const testUserId2 = 900002;
  let reviewId: number;

  beforeAll(async () => {
    await db.delete(flaggedReviews);
    await db.delete(reviewHelpfulVotes);
    await db.delete(reviewAnalytics).where(inArray(reviewAnalytics.userId, [testUserId1, testUserId2]));
    await db.delete(reviews).where(or(
      eq(reviews.fromUserId, testUserId1),
      eq(reviews.toUserId, testUserId2)
    ));
    ...
    await db.insert(users).values([
      {
        id: testUserId1,
        openId: "review-test-user-1",
        name: "Review Test Buyer",
        email: "review-buyer@example.test",
        role: "user",
        lastSignedIn: new Date(),
      },
      {
        id: testUserId2,
        openId: "review-test-user-2",
        name: "Review Test Seller",
        email: "review-seller@example.test",
        role: "user",
        lastSignedIn: new Date(),
      },
    ]);
